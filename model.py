...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        # flag pour choisir softmax-1 ou softmax standard
        self.use_softmax1 = getattr(config, 'use_softmax1', False)
        self.flash = hasattr(torch.nn.functional, 'scaled_dot_product_attention')
        if not self.flash or self.use_softmax1:
            self.flash = False
            self.register_buffer("bias", torch.tril(torch.ones(config.block_size, config.block_size))
                                        .view(1, 1, config.block_size, config.block_size))
            if self.use_softmax1:
                print("INFO: using softmax-1 attention (manual mode)")
            else:
                print("WARNING: using slow attention. Flash Attention requires PyTorch >= 2.0")

    def softmax1(self, att):
        """
        softmax-1(xi) = exp(xi) / (1 + sum(exp(xj))). Implementation of softmax1
        """
